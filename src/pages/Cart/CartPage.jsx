...
const CartPage = () => {
    ...
    const { cart = [], isLoading, isError, errorMsg } = useSelector((state) => state.carts);
    ...
    const fetchCart = useCallback(() => {
        try {
            ...
            toast.error(`Failed to fetch cart items: ${error.message || 'Server error'}`);
        }
    }, [dispatch]);
    ...
    return (
        <section className={`min-h-screen rounded-2xl overflow-hidden ${theme === "dark" ? "bg-gray-900 shadow-xl shadow-amber-500" : "bg-slate-0"} py-10 px-4 transition-colors duration-300`}>
                    ...
                    {cart.map((item) => (
                        ...
                        >
                            ...
                            <div className=" flex flex-col items-center sm:items-end gap-3">
                                <div className="flex items-center border rounded-lg overflow-hidden  sm:w-auto">
                                    <button onClick={() => {
                                        dispatch(
